...
func GetByIdForUpdate[T any](tc *TransContext, id int64, meta *TableMeta[T], skipLocked bool, viewColumns ...string) (*T, error) {
	...
	m.Eq(fieldId, id)
	return QueryOneMatcherForUpdate(tc, m, meta, skipLocked, viewColumns...)
}
...
/*
QueryPageListMatcherWithViewColumns 根据查询条件 Matcher 及 Pager 返回一页数据， 通过与 Matcher 有关的相关函数来构建查询条件
orders 可变参数：
...
*/
func QueryPageListMatcherWithViewColumns[T any](tc *TransContext, m Matcher, meta *TableMeta[T], viewColumns []string, page *Pager, orders ...*Order) ([]*T, error) {
	view := &View{
		viewColumns: viewColumns,
		include:     true,
	}
	return QueryPageListMatcherWithViewObj(tc, m, meta, view, page, orders...)
}
...
orders 可变参数：
可以传入一个、多个或者零个排序条件每个条件可以指定排序表字段名及是否是升序要求
*/
